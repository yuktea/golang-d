...
func HandleCommand(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		respondWithJSON(w, http.StatusBadRequest, ErrorResponse{ErrorMessage: "Error Invalid Payload (Not JSON)"})
		return
	}

	if data.Command == "" {
		respondWithJSON(w, http.StatusBadRequest, ErrorResponse{ErrorMessage: "Command not specified"})
		return
	}
...
func checkIfNotPost(w http.ResponseWriter, r *http.Request) bool {
	if r.Method != http.MethodPost {
		respondWithJSON(w, http.StatusMethodNotAllowed, ErrorResponse{ErrorMessage: "Method not allowed (can only use POST)"})
		return true
	}
	return false
}
